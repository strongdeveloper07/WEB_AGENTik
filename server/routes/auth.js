...
router.post('/register', (req, res) => {
  const { fullname, login, password, polis, phone, email, birthDate, passport, clinic } = req.body;
  ...
  try {
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(fullname, login, hashedPassword, polis,
           phone || null, email || null, birthDate || null, passport || null, clinic || null);
...
router.post('/login', (req, res) => {
  ...
  if (!user) return res.status(401).json({ error: 'Пользователь с таким логином не найден' });
  if (!bcrypt.compareSync(password, user.password)) return res.status(401).json({ error: 'Неверный пароль' });
  if (user.polis !== polis) return res.status(401).json({ error: 'Неверный номер полиса' });
  ...
  res.json({
    status: 'ok',
    access_token: token,
    user: { id: user.id, fullname: user.fullname, login: user.login, device_id: user.device_id || null }
  });
});

router.post('/bind-device', authenticateToken, (req, res) => {
  const { device_id } = req.body;
  if (!device_id) return res.status(400).json({ error: 'Укажите device_id' });
  db.prepare('UPDATE users SET device_id = ? WHERE id = ?').run(device_id, req.user.id);
